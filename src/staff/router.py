...
async def staff_register(user: UserSchema, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
...
def staff_get_all_users(db: Session = Depends(get_db)):
...
def staff_login(user: LoginSchema, request: Request, db: Session = Depends(get_db)):
    return controller.login(user, request, db)
...
def staff_authenticate(request: Request, db: Session = Depends(get_db)):
...
def staff_edit(id: int, user: UserUpdateSchema, user_logged: UserModel = Depends(is_authenticated), db: Session = Depends(get_db)):
    return controller.edit_user_by_id(id, user, user_logged, db)
...
async def staff_deactivate(id: int, background_tasks: BackgroundTasks, user_logged: UserModel = Depends(is_authenticated), db: Session = Depends(get_db)):
...
async def staff_activate(id: int, background_tasks: BackgroundTasks, user_logged: UserModel = Depends(is_authenticated), db: Session = Depends(get_db)):
...
def staff_delete(id: int, user_logged: UserModel = Depends(is_authenticated), db: Session = Depends(get_db)):
    return controller.delete_user_by_id(id, user_logged, db)
