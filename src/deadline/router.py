...
def set_deadline(deadline: DeadlineSchema, db: Session = Depends(get_db), user: UserModel = Depends(is_authenticated)):
    # Guard check
    if user.role.lower() != "hr":
        raise HTTPException(status_code=403, detail="Only HR can set ordering deadlines.")
    
    return controller.set_deadline(deadline, db, user)

###########################################################################################

#Create a route to get a deadline
@deadline_router.get("/{week_string}")
def get_deadline(week_string: str, db: Session = Depends(get_db), user: UserModel = Depends(is_authenticated)):
    return controller.get_deadline_by_week(week_string, db)

###########################################################################################

#Create a route to edit deadlines
@deadline_router.put("/edit/{id}",response_model=DeadlineSchema, status_code=status.HTTP_200_OK)
async def edit_deadline_by_id(id: int, deadline: DeadlineSchema, db: Session = Depends(get_db), user:UserModel = Depends(is_authenticated)):
    # Guard check
    if user.role.lower() != "hr":
...
def delete_deadline_by_id(id: int, db: Session = Depends(get_db), user:UserModel = Depends(is_authenticated)):
    # Guard check
    if user.role.lower() != "hr":
        raise HTTPException(status_code=403, detail="Only HR can delete ordering deadlines.")
    
    return controller.delete_deadline_by_id(id, db, user)